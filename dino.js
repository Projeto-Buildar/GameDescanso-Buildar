...
const collisionBoxElem = document.querySelector("[data-collision-box]"); // Seleciona a área de colisão
...
export function getDinoRect() {
  return collisionBoxElem.getBoundingClientRect();
}

// Função para definir a imagem do dinossauro quando ele perde
export function setDinoLose() {
  dinoElem.querySelector('.dino-img').src = "imgs/viviTriste.png";
}

// Função para controlar a animação de corrida
function handleRun(delta, speedScale) {
  if (isJumping) {
    dinoElem.querySelector('.dino-img').src = `imgs/viviJump.png`;
    return;
  }

  if (currentFrameTime >= FRAME_TIME) {
    dinoFrame = (dinoFrame + 1) % DINO_FRAME_COUNT;
    dinoElem.querySelector('.dino-img').src = `imgs/sprite_0${dinoFrame}.png`;
